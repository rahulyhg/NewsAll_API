...
//                    . " '$api_key', '$firebase_reg_id')";
    ...
    public function getSources($category_id, $language_ids) {
   
			$languages_array = json_decode($language_ids, TRUE);
			...
			$i = 0;
			$len = count($languages_array);
			foreach ($languages_array as $key) {
				
				if ($i == $len - 1) {
					$condition .= "id_language = ".$key['id_language']." ";
				}else{
					$condition .= "id_language = ".$key['id_language']." OR ";
				}
					// …
				$i++;
				}
